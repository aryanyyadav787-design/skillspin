...
export const LandingPage: React.FC<LandingPageProps> = ({ onLogin }) => {
  return (
    <div className="min-h-screen bg-[#F9FAFB] font-sans selection:bg-yellow-200 overflow-x-hidden flex flex-col text-[#1e1e1e]">
      ...
      {/* Navbar */}
      <nav className="flex justify-between items-center px-6 md:px-12 py-8 max-w-[1400px] mx-auto w-full z-50">
        <div className="flex items-center gap-2 cursor-pointer">
            <span className="text-3xl font-extrabold tracking-tight flex items-center gap-1">
                Spin 
            </span>
        </div>
        <div className="hidden md:flex items-center gap-10 font-bold text-gray-500 text-[15px]">
            <a href="#" className="hover:text-black transition-colors">Features</a>
            <a href="#" className="hover:text-black transition-colors">Company</a>
            <a href="#" className="hover:text-black transition-colors">Pricing</a>
        </div>
        <div className="flex items-center gap-6">
            <button onClick={onLogin} className="text-[15px] font-bold text-black hover:text-gray-600 transition-colors">Log in</button>
            <button 
                onClick={onLogin}
                className="bg-[#1e1e1e] text-white px-6 py-3 rounded-2xl text-[15px] font-bold hover:scale-105 hover:shadow-xl transition-all duration-300"
            >
                Try for free
            </button>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center md:items-start pt-10 pb-20 relative max-w-[1600px] mx-auto w-full">
        ...
        <div className="relative w-full h-[400px] md:h-[500px] mb-8 hidden md:flex justify-center items-center perspective-1000">
            ...
            <div className="absolute z-10 hover:z-50 transition-all duration-500 hover:scale-110 cursor-pointer group"
            ...
            <div className="absolute z-20 hover:z-50 transition-all duration-500 hover:scale-110 cursor-pointer group"
            ...
            <div className="absolute z-30 hover:z-50 transition-all duration-500 hover:scale-110 cursor-pointer group"
        ...
        <div className="text-left z-40 relative px-6 md:px-12 mt-4 max-w-5xl">
            <h1 className="text-[3.5rem] md:text-[6.5rem] font-extrabold text-[#1e1e1e] tracking-tighter leading-[1] mb-6 md:mb-8">
                ...
                learning any <span className="relative inline-block">
                    skill
                </span>
            </h1>
            ...
            <div className="relative inline-block group">
                 <div className="absolute inset-0 bg-gray-200 rounded-full blur-xl opacity-0 group-hover:opacity-50 transition-opacity duration-500"></div>
                 {/* Adjusted position for the hint arrow since text is left aligned now, pointing up towards cards */}
                 <div className="hidden md:block absolute left-[320px] -top-32 text-gray-300 text-sm font-handwriting transform -rotate-6">
                    <CornerRightDown className="inline-block mr-2 transform rotate-[225deg] mb-2" />
                    try hovering <br/> over the cards :)
                </div>
            </div>
        </div>
